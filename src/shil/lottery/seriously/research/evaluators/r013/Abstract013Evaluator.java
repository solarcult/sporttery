 ...
 */
public abstract class Abstract013Evaluator implements Guess013{
		...
		public PredictResultAnalyze call() throws Exception {
			...
			if(result>=0){
				if(vsTeam.getMatch_Result() == result){
					...
					predictResultAnalyze.getResult013Frequency().addValue(Abstract013Evaluator.Bingo+AnalyzeUtil.Connect+result);
				}else{
					predictResultAnalyze.getBingoFrequency().addValue(Abstract013Evaluator.NotBingo);
					predictResultAnalyze.getResult013Frequency().addValue(Abstract013Evaluator.NotBingo+AnalyzeUtil.Connect+result+"->"+vsTeam.getMatch_Result());
				}
			}else if(result == -1){
