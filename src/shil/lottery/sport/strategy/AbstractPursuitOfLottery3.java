...
public abstract class AbstractPursuitOfLottery3 implements PursuitOfLottery3{

public abstract double getLiRunU(int u,double[] ... dmatrix);
...
//		String[] leagues = StrategyUtils.convert2leagues(vsTeams);
		/*业务逻辑*/
		//初始化两对胜平负描述
		...
		{
			//核心胜率公式
			lirun[u] = getLiRunU(u, n_shouyi,n_win,n_conf);
			allinOne.add(new WCPBean(score_status[u], lirun[u], " shouyi: " + n_shouyi[u] + " ; win: " + n_win[u] + " ; conf: " +n_conf[u]));
			Collections.sort(allinOne);
			allinOne = StrategyUtils.subList(allinOne, 0, (allinOne.size()<64)?allinOne.size():64);
		}
		
		Collections.sort(allinOne);
		
		return StrategyUtils.subList(allinOne, 0, (allinOne.size()<64)?allinOne.size():64);
	}
